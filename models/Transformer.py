...
class LayerNorm(nn.Module):
    ...
    def forward(self, x):
        ...
        x = (x - u) / torch.sqrt(s + float(self.variance_epsilon))
...
class Output_Residual_Norm(nn.Module):
    def __init__(self, config):
        ...
        self.LayerNorm = LayerNorm(config.hidden_size, eps=float(config.layer_norm_eps))
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
...
class TransEncoder(nn.Module):
    def __init__(self, config, layer_num, get_max_lens, hidden_size):
        ...
        encoder_layer = nn.TransformerEncoderLayer(
            d_model=hidden_size,  # 输入的隐藏层大小
            nhead=config.num_attention_heads,  # 多头注意力头的数量
            dim_feedforward=config.intermediate_size,  # 前馈网络的维度
            dropout=config.attention_probs_dropout_prob  # Dropout 概率
        )
        
        # 构建 Transformer 编码器，包含 layer_num 层
        self.transformer_encoder = nn.TransformerEncoder(encoder_layer, num_layers=layer_num)

    def forward(self, x, attention_mask=None):
        # 将位置嵌入加到输入张量中
        position_ids = torch.arange(x.size(1), device=x.device).unsqueeze(0).expand_as(x[:, :, 0])
