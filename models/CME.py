...
class ACME(torch.nn.Module):
    ...
    def forward(self, features_t, features_a, key_padding_mask): 
        for mod in self.layers:
            features_t, features_a = mod(features_t, features_a, key_padding_mask)
        return features_t, features_a
...
class ACMELayer(torch.nn.Module):
    def __init__(self, feature_size, nheads=4, dropout=0.3, no_cuda=False):
        ...
        self.linear_cat = nn.Linear(feature_size, feature_size)
    ...
    def forward(self, features_t, features_a, key_padding_mask):
        ...
        feat_t,feat_a = features_t, features_a
        self_feat_t, self_feat_a = self._self_att(feat_t, feat_a, key_padding_mask)
        self_feat_t, self_feat_a = self.norm_t(feat_t + self_feat_t), self.norm_a(feat_a + self_feat_a)

        cross_feat_t, cross_feat_a = self._cross_att(self_feat_t, self_feat_a, key_padding_mask)
        cross_feat_t, cross_feat_a = self.norm_t1(feat_t + self_feat_t + cross_feat_t), self.norm_a1(feat_a + self_feat_a + cross_feat_a)

        full_feat_t, full_feat_a = self._full_con(cross_feat_t, cross_feat_a)
        full_feat_t, full_feat_a = self.norm_t2(feat_t + cross_feat_t + full_feat_t), self.norm_a2(feat_a + cross_feat_a + full_feat_a)

        return full_feat_t, full_feat_a

    def _self_att(self, features_t, features_a, key_padding_mask):
        ...
        return self.dropout_t(feat_t), self.dropout_a(feat_a)

    def _cross_att(self, features_t, features_a, key_padding_mask):
        # feat_t1 = self.cross_att_tv(features_t,features_v,features_v, key_padding_mask)[0]
        feat_t = self.cross_att_ta(features_t,features_a,features_a, key_padding_mask)[0]
        ...
        return self.dropout_t1(feat_t), self.dropout_a1(feat_a)
    
    def _full_con(self, features_t, features_a):
        ...
        return self.dropout_t3(feat_t), self.dropout_a3(feat_a)
